...
import org.tl.nettyServer.servers.annotations.Anonymous;
import org.tl.nettyServer.servers.buf.BufFacade;
import org.tl.nettyServer.servers.flex.messaging.io.ObjectProxy;
import org.tl.nettyServer.servers.io.amf.AMF;
import org.tl.nettyServer.servers.io.amf.Output;
import org.tl.nettyServer.servers.io.object.IOutput;
import org.tl.nettyServer.servers.io.object.RecordSet;
import org.tl.nettyServer.servers.io.object.Serializer;
import org.tl.nettyServer.servers.io.object.UnsignedInt;
import org.tl.nettyServer.servers.util.HexDump;
import org.tl.nettyServer.servers.util.XMLUtils;
 ...
 * @author Harald Radi (harald.radi@nme.aI)
 * @see AMF3
 * @see Input3
 */
public class Output3 extends Output implements IOutput {
     ...
     *
     * @param buf instance of BufFacade
     * @see BufFacade
     */
    public Output3(BufFacade buf) {
        super(buf);
        amf3_mode = 0;
     ...
     * @return BufFacade
     */
    protected BufFacade getBuffer() {
        return buf;
    }

    protected void writeAMF3() {
        if (amf3_mode == 0) {
            buf.writeByte(AMF.TYPE_AMF3_OBJECT);
        }
    }
    ...
    public void writeBoolean(Boolean bol) {
        writeAMF3();
        buf.writeByte(bol ? AMF3.TYPE_BOOLEAN_TRUE : AMF3.TYPE_BOOLEAN_FALSE);
    }
    ...
    protected void putInteger(long value) {
        ...
        if (value < 128) {
            ...
            buf.writeByte((byte) (((value >> 14) & 0x7F) | 0x80));
            buf.writeByte((byte) (((value >> 7) & 0x7F) | 0x80));
            buf.writeByte((byte) (value & 0x7F));
        } else if (value < 1073741824) {
            buf.writeByte((byte) (((value >> 22) & 0x7F) | 0x80));
            buf.writeByte((byte) (((value >> 15) & 0x7F) | 0x80));
            buf.writeByte((byte) (((value >> 8) & 0x7F) | 0x80));
    ...
    public void writeNumber(Number num) {
        ...
        if (num.longValue() < AMF3.MIN_INTEGER_VALUE || num.longValue() > AMF3.MAX_INTEGER_VALUE) {
            ...
            buf.writeByte(AMF3.TYPE_NUMBER);
            buf.writeDouble(num.doubleValue());
        } else if (num instanceof Long || num instanceof Integer || num instanceof Short || num instanceof Byte) {
            buf.writeByte(AMF3.TYPE_INTEGER);
    ...
    public void writeString(String string) {
        writeAMF3();
        buf.writeByte(AMF3.TYPE_STRING);
        if ("".equals(string)) {
            putInteger(1);
        } else {
            final byte[] encoded = encodeString(string);
    ...
    private void writePrimitiveByteArray(byte[] bytes) {
        writeAMF3();
        this.buf.writeByte(AMF3.TYPE_BYTEARRAY);
    ...
    public void writeObject(Object object) {
        ...
        writeAMF3();
        buf.writeByte(AMF3.TYPE_OBJECT);
    ...
    public void writeByteArray(ByteArray array) {
        writeAMF3();
        buf.writeByte(AMF3.TYPE_BYTEARRAY);
        if (hasReference(array)) {
            putInteger(getReferenceId(array) << 1);
            return;
        }
    ...
    public void writeVectorInt(Vector<Integer> vector) {
        ...
        writeAMF3();
        buf.writeByte(AMF3.TYPE_VECTOR_INT);
    ...
    public void writeVectorUInt(Vector<Long> vector) {
        ...
        writeAMF3();
        buf.writeByte(AMF3.TYPE_VECTOR_UINT);
        if (hasReference(vector)) {
            putInteger(getReferenceId(vector) << 1);
            return;
        }
    ...
    public void writeVectorNumber(Vector<Double> vector) {
        ...
        buf.writeByte(AMF3.TYPE_VECTOR_NUMBER);
        if (hasReference(vector)) {
            putInteger(getReferenceId(vector) << 1);
            return;
        }
    ...
    public void writeVectorObject(Vector<Object> vector) {
        ...
        buf.writeByte(AMF3.TYPE_VECTOR_OBJECT);
        if (hasReference(vector)) {
            putInteger(getReferenceId(vector) << 1);
            return;
        }
