...
package org.tl.nettyServer.servers.io.amf;
...
import org.tl.nettyServer.servers.io.object.*;
 ...
 */
public class Output extends BaseOutput implements IOutput {
    ...
    public void writeArray(Object array) {
        if (array != null) {
            if (!checkWriteReference(array)) {
                storeReference(array);
                buf.writeByte(AMF.TYPE_ARRAY);
                buf.writeInt(Array.getLength(array));
    ...
    public void writeMap(Map<Object, Object> map) {
        ...
        if (!checkWriteReference(map)) {
            storeReference(map);
            buf.writeByte(AMF.TYPE_MIXED_ARRAY);
            int maxInt = -1;
            ...
            buf.writeBytes(AMF.END_OF_OBJECT_SEQUENCE);
        }
    }
    ...
    public void writeRecordSet(RecordSet recordset) {
        if (!checkWriteReference(recordset)) {
            ...
            buf.writeByte(AMF.TYPE_CLASS_OBJECT);
            putString("RecordSet");
            // Serialize
    ...
    public void writeBoolean(Boolean bol) {
        buf.writeByte(AMF.TYPE_BOOLEAN);
        buf.writeByte(bol ? AMF.VALUE_TRUE : AMF.VALUE_FALSE);
    }
    ...
    public void writeDate(Date date) {
        buf.writeByte(AMF.TYPE_DATE);
        buf.writeDouble(date.getTime());
        buf.writeShort((short) (TimeZone.getDefault().getRawOffset() / 60 / 1000));
    }
    ...
    public void writeReference(Object obj) {
        ...
        buf.writeByte(AMF.TYPE_REFERENCE);
        buf.writeShort(getReferenceId(obj));
    }
    ...
    public void writeString(String string) {
        ...
        if (len < AMF.LONG_STRING_LENGTH) {
            buf.writeByte(AMF.TYPE_STRING);
            // write unsigned short
            buf.writeByte((byte) ((len >> 8) & 0xff));
            buf.writeByte((byte) (len & 0xff));
        } else {
            buf.writeByte(AMF.TYPE_LONG_STRING);
            buf.writeInt(len);
        }
     ...
     */
    public static void putString(BufFacade buf, String string) {
        final byte[] encoded = encodeString(string);
        if (encoded.length < AMF.LONG_STRING_LENGTH) {
            // write unsigned short
            buf.writeByte((byte) ((encoded.length >> 8) & 0xff));
            buf.writeByte((byte) (encoded.length & 0xff));
        } else {
