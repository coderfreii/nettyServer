package org.tl.nettyServer.servers.message;


import org.tl.nettyServer.servers.codec.HTTPCodecUtil;
import org.tl.nettyServer.servers.request.HTTPRequest;
import org.tl.nettyServer.servers.response.HTTPResponse;
import org.tl.nettyServer.servers.message.HTTPHeaderDateFormat;
...
public class HTTPHeaders {
    ...
    private void removeHeader0(int h, int i, String name) {
        ...
        for (; ; ) {
            Entry next = e.next;  //store
            if (next == null) {  //break at end of the chain
                break;
            }
            if (next.hash == h && eq(name, next.key)) { //if eq remove
                e.next = next.next;
                next.remove();
            } else { // not eq continue loop
                e = next;
            }
        }
    }
