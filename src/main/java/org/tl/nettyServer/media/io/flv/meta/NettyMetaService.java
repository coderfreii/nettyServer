 ...
 */
public class NettyMetaService implements INettyMetaService {

    protected static Logger log = LoggerFactory.getLogger(NettyMetaService.class);
     ...
     */
    public NettyMetaService() {
        super();
    }

    public NettyMetaService(File poFil) {
        this();
        this.file = poFil;
    }
    ...
    private static INettyTag injectMetaData(IMetaData<?, ?> meta, INettyTag tag) {
        ...
        BufFacade tmpBody = out.buf();
        int tmpBodySize = out.buf().readableBytes();
        int tmpPreviousTagSize = tag.getPreviousTagSize();
        return new NettyTag(IoConstants.TYPE_METADATA, 0, tmpBodySize, tmpBody, tmpPreviousTagSize);
    }
    ...
    private static INettyTag injectMetaCue(IMetaCue meta, INettyTag tag) {
        ...
        return new NettyTag(IoConstants.TYPE_METADATA, tmpTimestamp, tmpBodySize, tmpBody, tmpPreviousTagSize);
    }
