package org.tl.nettyServer.media.io.mp4;/*
...
import org.tl.nettyServer.media.io.mp4.MP4Frame;
...
public class MP4Reader implements IoConstants, ITagReader, IKeyFrameDataAnalyzer {
    ...
    public MP4Reader(File f) throws IOException {
        ...
        if (f.exists() && f.canRead()) {
            ...
            isoFile = new IsoFile(Paths.get(f.toURI()).toString());
    ...
    ITag createFileMeta() {
        ...
        BufFacade buf = BufFacade.buffer(1024);
    ...
    private void createPreStreamingTags(int timestamp, boolean clear) {
        ...
        if (hasVideo) {
            //video tag #1
            body = BufFacade.buffer(41);
            body.writeBytes(PREFIX_VIDEO_CONFIG_FRAME); //prefix
            if (videoDecoderBytes != null) {
                ...
                body.writeBytes(videoDecoderBytes);
            }
            tag = new Tag(IoConstants.TYPE_VIDEO, timestamp, body.readableBytes(), null, 0);
            tag.setBody(body);
            //add tag
            firstTags.add(tag);
        }
        ...
        if (hasAudio) {
            ...
            if (audioDecoderBytes != null) {
                ...
                body = BufFacade.buffer(audioDecoderBytes.length + 3);
                body.writeBytes(PREFIX_AUDIO_CONFIG_FRAME); //prefix
                body.writeBytes(audioDecoderBytes);
                body.writeByte((byte) 0x06); //suffix
                tag = new Tag(IoConstants.TYPE_AUDIO, timestamp, body.readableBytes(), null, 0);
                tag.setBody(body);
                //add tag
                firstTags.add(tag);
            } else {
        ...
        if (!frames.isEmpty()) {
            try {
                ...
                if (frame != null) {
                    ...
                    // chunk the data
                    BufFacade payload = BufFacade.wrappedBuffer(data.array());
                    // create the tag
                    tag = new Tag(type, time, payload.readableBytes(), payload, prevFrameSize);
    ...
    public void close() {
        ...
        if (dataSource != null) {
            try {
                dataSource.close();
                List<MediaDataBox> boxes = isoFile.getBoxes(MediaDataBox.class);
                for (MediaDataBox box : boxes) {
//                    box.close();
                }
            } catch (IOException e) {
