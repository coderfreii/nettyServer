...
package org.tl.nettyServer.media.io.ts;
...
public class TransportStreamUtils {
    ...
    public static void fillPAT(byte[] data, int pos, long patCounter) {
        //sync_byte  									8bit 	0x47                   47
        //transport_error_indicator 		 			1		0x00
        //payload_unit_start_indicator 是否为 1，表明具体负载数据前是否存在 pointer_field
        //payload_unit_start_indicator 					1		0x01
        //transport_priority                            1       0x00
        //pid                                           13      0x0000    PAT的pid     4000
        //transport_scrambling_control					2       0x00
        //adaptation_field_control						2		0x01
        //continuity_counter							4		0x00				   10                  4

        //payload
        //没有adaptation field
        //pointer_field						            8       0x00				   00


        //table_id										8  		0x00				   00                  1
        //section_syntax_indicator						1       0x01
        //zero											1       0x00
        //reserved										2       0x03			       B
        //section_length								12      0x0D				   00D                 3
        //transport_stream_id							16		0x0001				   0001
        //reserved										2       0x03
        //version_number                                5       0x00
        //current_next_indicator						1       0x01				   C1                   6
        //section_number								8       0x00				   00
        //last_section_number							8       0x00				   00
        //program_number								16      0x0001				   0001                 10
        //reserved										3		0x07
        ...
        int fourthByteIndex = pos + 3;
        //取出高4位  0xXXXX0000
        byte fourthByteHigh4bits = (byte) (data[fourthByteIndex] & 0xF0);
        //拼接
        data[fourthByteIndex] = (byte) (fourthByteHigh4bits | (byte) (patCounter & 0xF));
    ...
    public static void fillPMT(byte[] data, int startPos, long pmtCounter, int videoPid, int audioPid, int videoStreamType, int audioStreamType) {
        int PMT_PID = 0xFF1; // pmt id
        int pos = 0;

        data[startPos + pos] = 0x47; // sync_byte
        pos++;

        int high3bits = BinaryUtils.bits2Int(new int[]{0, 1, 0}, 5);
        data[startPos + pos] = (byte) (high3bits + (PMT_PID >> 8 & 0x1F));
        String s = BinaryUtils.printBinaryFromInt(data[startPos + pos], 8, 0, 8);
        pos++;


        data[startPos + pos] = (byte) (PMT_PID & 0xFF);
        pos++;

        byte fourthByteHigh4bits = 0x10;
        data[startPos + pos] = (byte) (int) (fourthByteHigh4bits + (pmtCounter & 0xF));
        pos++;

        data[startPos + pos] = 0;   //pointer_field
        pos++;

        data[startPos + pos] = 2;  //tableId  0x02
        pos++;

        data[startPos + pos] = (byte) 0xB0;    //默认值
        pos++;

        data[startPos + pos] = 0;
        pos++;                                 //section_length  16bits段长度,从program_number开始,到CRC_32(包含)的字节总数.

        int k = pos;
        int program_number = 0x01;
        data[startPos + pos] = (byte) (program_number >> 8);
        pos++;

        data[startPos + pos] = (byte) (program_number & 0x0F);              //program_number
        pos++;

        data[startPos + pos] = (byte) 0xC1;
        pos++;

        //section_number有子表时会递增  子表的概念是，具有相同的 table_id 和 table_id_extension， 不同 section_number 来区分   0开始
        data[startPos + pos] = 0;                   //section_number
        pos++;

        data[startPos + pos] = 0;                   //last_section_number     第8个
        pos++;


        //选取PRC_ID  当有视频时 选取视频PID作为PCR_ID
        int m = videoPid;   //0x100
        if (videoStreamType == TransportStreamUtils.STREAM_TYPE_VIDEO_UNKNOWN) {
            m = audioPid;//0x101
        }
        ...
        int reserved = 0xE0;
        int PCR_PID = m;
        data[startPos + pos] = (byte) (reserved + (PCR_PID >> 8));
        pos++;


        data[startPos + pos] = (byte) (PCR_PID & 0xFF);
        pos++;                                              //PCR_PID
        ...
        int program_info_length = 0x000;
        data[startPos + pos] = (byte) (0xF0 + program_info_length >> 8);
        pos++;

        data[startPos + pos] = (byte) (program_info_length & 0x0FF);
        pos++;                                      //program_info_length

        //视频节目信息
        if (videoStreamType != TransportStreamUtils.STREAM_TYPE_VIDEO_UNKNOWN) {
            data[startPos + pos] = (byte) videoStreamType;        //流类型,标志是Video还是Audio还是其他数据。这里是MPEG2视频类型
            pos++;

            int __reserved = 0xE0;
            data[startPos + pos] = (byte) (__reserved + (videoPid >> 8));
            pos++;

            data[startPos + pos] = (byte) (videoPid & 0xFF);
            pos++;

            int ____reserved = 0xF0;
            int ES_info_length = 0x000;
            data[startPos + pos] = (byte) (____reserved + ES_info_length >> 8);
            pos++;

            data[startPos + pos] = (byte) (ES_info_length & 0xFF);
            pos++;
        }
        //音频节目信息
        if (audioStreamType != TransportStreamUtils.STREAM_TYPE_AUDIO_UNKNOWN) {  //同上
        ...
        int sectionLengthValue = pos - k + 4;  //计算section_length
        int sectionLengthIndex = (startPos + k - 1);
        data[sectionLengthIndex] = (byte) (data[sectionLengthIndex] + sectionLengthValue);  //赋值


        //length 从tableId（包含） 到crc32（不包含）之间的字节数
        int length = sectionLengthValue + 3 - 4;
        int crc32 = doCRC32(data, startPos + 5, length);
        //这个方法生成的是逆顺序的的四个字节
        for (int i = 0; i < 4; i++) {
