...
import org.tl.nettyServer.media.io.mp4.MP4Reader;
...
public class M4AReader implements IoConstants, ITagReader {
    ...
    private void createPreStreamingTags() {
        ...
        if (audioDecoderBytes != null) {
            BufFacade body = BufFacade.buffer(audioDecoderBytes.length + 3);
            body.writeBytes(new byte[] { (byte) 0xaf, (byte) 0 }); //prefix
            if (log.isDebugEnabled()) {
            ...
            ITag tag = new Tag(IoConstants.TYPE_AUDIO, 0, body.readableBytes(), null, prevFrameSize);
            tag.setBody(body);
            //add tag
            firstTags.add(tag);
        } else {
