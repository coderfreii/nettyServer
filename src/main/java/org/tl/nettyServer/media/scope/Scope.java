...
import org.tl.nettyServer.media.exception.ScopeException;
import org.tl.nettyServer.media.jmx.mxbeans.ScopeMXBean;
    ...
    private final class ConcurrentScopeSet extends ConcurrentSkipListSet<IBasicScope> {
        ...
        public IBasicScope getBasicScope(ScopeType type, String name) {
            ...
            if (ScopeType.UNDEFINED.equals(type)) {
                ...
                scope = stream().filter(child ->
                        child != null && child.getType().equals(type.toString()) && name.equals(child.getName())).findFirst();
            }
            if (scope.isPresent()) {
                return scope.get();
            }
