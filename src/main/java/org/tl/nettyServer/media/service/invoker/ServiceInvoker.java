...
package org.tl.nettyServer.media.service.invoker;

import lombok.extern.slf4j.Slf4j;
import org.tl.nettyServer.media.Red5;
import org.tl.nettyServer.media.annotations.DeclarePrivate;
import org.tl.nettyServer.media.annotations.DeclareProtected;
import org.tl.nettyServer.media.exception.ClientDetailsException;
import org.tl.nettyServer.media.exception.MethodNotFoundException;
import org.tl.nettyServer.media.exception.NotAllowedException;
...
import org.tl.nettyServer.media.util.ReflectionUtils;
...
public class ServiceInvoker implements IServiceInvoker {
    ...
    String normalizeServiceMethodName(IServiceCall call, Object service) {
    ...
    Object[] fillArgs(IServiceCall call, IConnection conn) {
        ...
        return argsWithConnection;
    }

    public boolean invoke(IServiceCall call, Object service) {
        IConnection conn = Red5.getConnectionLocal();
        String methodName = normalizeServiceMethodName(call, service);
        // build an array with the incoming args and the current connection as the first element
        Object[] args = call.getArguments();
        Object[] argsWithConnection = fillArgs(call, conn);

        // find the method
        Object[] methodResult = resolveMethod(service, methodName, args, argsWithConnection);
        ...
        if (methodResult.length == 0 || methodResult[0] == null) {
            ...
            call.setStatus(ServiceCall.STATUS_METHOD_NOT_FOUND);
        ...
        return doInvoke(call, methodResult, conn, service);
    }

    Object[] resolveMethod(Object service, String methodName, Object[] args, Object[] argsWithConnection) {
        Object[] methodResult;
    ...
    boolean doInvoke(IServiceCall call, Object[] methodResult, IConnection conn, Object service) {
        Object result = null;
        ...
        try {
            ...
            if (method.getReturnType().equals(Void.TYPE)) {
                ...
                call.setStatus(ServiceCall.STATUS_SUCCESS_VOID);
            } else {
                result = method.invoke(service, params);
                log.debug("result: {}", result);
                call.setStatus(result == null ? ServiceCall.STATUS_SUCCESS_NULL : ServiceCall.STATUS_SUCCESS_RESULT);
            }
            ...
            call.setStatus(ServiceCall.STATUS_INVOCATION_EXCEPTION);
