...
public class RtmpProtocolState {
    ...
    /**
     * Map for channelsInfo, keyed by channel id.
     */
    private final transient ConcurrentMap<Integer, ChannelInfo> channelsInfo = new ConcurrentHashMap<>(3, 0.9f, 1);
    ...
    private ChannelInfo getChannelInfo(int channelId) {
        ChannelInfo info = channelsInfo.putIfAbsent(channelId, new ChannelInfo());
        if (info == null) {
            info = channelsInfo.get(channelId);
        }
        return info;
    }
     ...
     */
    public Packet getIncompleteReadPacket(int channelId) {
