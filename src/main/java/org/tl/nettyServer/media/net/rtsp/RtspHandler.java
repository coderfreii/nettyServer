 ...
 * @author pengliren
 */
public final class RtspHandler {

    private Logger log = LoggerFactory.getLogger(RtspHandler.class);

    private Pattern rtspTransportPattern = Pattern.compile(".*client_port=(\\d*)-(\\d*).*");

    public ConcurrentHashMap<String, RTPPlayer> rtpSocketMaps = new ConcurrentHashMap<String, RTPPlayer>();
     ...
     * @throws Exception
     */
    public boolean handleRtspMethod(HTTPRequest request, HTTPResponse response) throws Exception {
        ...
        if (request.getMethod() == HTTPMethod.OPTIONS) {
            ...
            String path = request.getUri().substring(1);
            String[] segments = path.split("/");
            IScope scope = null;
            IGlobalScope global = server.lookupGlobal(null, segments[segments.length - 2]);
            ...
            scope = context.resolveScope(global, segments[segments.length - 2]);
            if (scope == null) {
                return false;
            }
            describe(request, response, scope);
     ...
     * @param response
     */
    public void describe(HTTPRequest request, HTTPResponse response, IScope scope) {

        try {
            RTSPMinaConnection conn = (RTSPMinaConnection) Red5.getConnectionLocal();
            RTSPConnectionConsumer rtspConsumer = createPlayStream(request.getUri(), scope);
     ...
     * @throws Exception
     */
    public void announce(HTTPRequest request, HTTPResponse response) throws Exception {
     ...
     * @param response
     */
    public void getParameter(HTTPRequest request, HTTPResponse response) {
     ...
     * @param response
     */
    public void setParameter(HTTPRequest request, HTTPResponse response) {
     ...
     * @param response
     */
    public void pause(HTTPRequest request, HTTPResponse response) {
     ...
     * @throws IOException
     */
    public void play(HTTPRequest request, HTTPResponse response) throws Exception {
     ...
     * @param response
     */
    public void record(HTTPRequest request, HTTPResponse response) {
     ...
     * @throws Exception
     */
    public void setup(HTTPRequest request, HTTPResponse response) throws Exception {
     ...
     * @throws Exception
     */
    private void setupInbound(HTTPRequest request, HTTPResponse response) throws Exception {
     ...
     * @throws Exception
     */
    private void setupOutbound(HTTPRequest request, HTTPResponse response) throws Exception {
     ...
     * @return
     */
    private boolean bindVideoPort(int[] pair) {
        boolean flag = true;
     ...
     * @return
     */
    private boolean bindAudioPort(int[] pair) {
        boolean flag = true;
    ...
    public void teardown(HTTPRequest request, HTTPResponse response) {
     ...
     * @throws SdpException
     */
    public String configureSDP() throws Exception {
     ...
     */
    private RTSPPushProxyStream createPublishStream(String url, RTSPMinaConnection conn) throws URISyntaxException {
        ...
        IScope scope = null;
        IGlobalScope global = server.lookupGlobal(null, segments[0]);
        log.trace("Global lookup result: {}", global);
        if (global == null) {
            return null;
        }
        final IContext context = global.getContext();
        // TODO optimize this to use Scope instead of Context
        scope = context.resolveScope(global, segments[0]);
     ...
     * @throws URISyntaxException
     */
    private RTSPConnectionConsumer createPlayStream(String url, IScope scope) throws URISyntaxException {
