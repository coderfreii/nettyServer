...
package org.tl.nettyServer.media.persistence;
...
import org.tl.nettyServer.media.so.SharedObject;
import org.tl.nettyServer.media.util.ServletUtils;
...
public class FilePersistence extends RamPersistence {
    ...
    private IPersistable doLoad(String name, IPersistable object) {
        ...
        try {
            BufFacade buf = BufFacade.buffer(input.available());
            try {
    ...
    protected boolean saveObject(IPersistable object) {
        ...
        if (result) {
            ...
            try {
                ...
                buf = BufFacade.buffer(initialSize);
