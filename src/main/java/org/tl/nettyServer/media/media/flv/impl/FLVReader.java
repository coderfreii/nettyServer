...
public class FLVReader implements IoConstants, IKeyFrameDataAnalyzer, ITagReader, Closeable {
    ...
    protected void setCurrentPosition(long pos) {
        ...
        if (!useLoadBuf) {
            in.setIndex((int) pos, in.writerIndex());
            return;
        }
        try {
    ...
    public synchronized KeyFrameMeta analyzeKeyFrames() {
        ...
        while (hasMoreTags()) {
            ...
            //this duration between flv start time 0 and the time point playing at this tag data cost
            duration = tmpTag.getTimestamp();
            if (tmpTag.getDataType() == IoConstants.TYPE_VIDEO) {
                if (audioOnly) {
                    audioOnly = false;
                    // if video exist we don‘t need the audio
            ...
            //15 is  previousTagSize plus to flv tag header size
            long newPosition = pos + tmpTag.getBodySize() + 15;
            //         previousTagSize plus to flv tag header size and bodySize
            //--------    previousTagSize  ----         whole flv tag           -----
            //-------- /\                                                      /\
            //--------  |                                                       |
            //--------  |                                                       |
            //-------- pos                                                  newPosition
...
//        		}
        // previous tag's size
        int previousTagSize = in.readInt();   // 4B

        // start of the flv tag
        byte dataType = in.readByte();     // 1B
        ...
        int bodySize = in.readUnsignedMedium();  //  3B

        int result = in.readInt();   //  4B

        int timestamp = (result >>> 8);

        int timestampExtended = ((result & 0x000000ff) << 24);

        //高低位拼接起来    0000 0001 | 1000 0000  ->  1000 0001
        timestamp = (timestamp | timestampExtended);

        if (log.isDebugEnabled()) {
            int streamId = in.readUnsignedMedium();
