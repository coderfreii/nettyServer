...
import org.tl.nettyServer.media.media.flv.meta.MetaData;
import org.tl.nettyServer.media.media.flv.meta.MetaService;
...
public class FLV implements IFLV {
    ...
    public FLV(File file, boolean generateMetadata) {
        ...
        if (generateMetadata) {
            try {
                FLVReader_ reader = new FLVReader_(this.file);
                ITag tag = null;
                int count = 0;
    ...
    public ITagReader getReader() throws IOException {
        ...
        if (null == ic || (null == ic.getByteBuffer())) {
            if (file.exists()) {
                ...
                reader = new FLVReader_(file, generateMetadata);
            ...
            reader = new FLVReader_(fileData, generateMetadata);
        }
        return reader;
    }
