...
public class Crc {
    static boolean CONFIG_HARDCODED_TABLES = false;
    static boolean CONFIG_SMALL = false;
    static int CRC_TABLE_SIZE = 257;
    //AVCRC
    static int[][]  av_crc_table;
    static {
        if(CONFIG_SMALL) {
            CRC_TABLE_SIZE = 1024;
        }
        av_crc_table = new int[AVCRCId.AV_CRC_MAX.getValue()][CRC_TABLE_SIZE];
        av_crc_table[9] = new int[]{0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B, 0x12, 0x15, 0x38, 0x3F, 0x36, 0x31,
        ...
        av_crc_table[8] = new int[]{0x00, 0x1D, 0x3A, 0x27, 0x74, 0x69, 0x4E, 0x53, 0xE8, 0xF5, 0xD2, 0xCF,
        ...
        av_crc_table[2] = new int[]{ 0x0000, 0x0580, 0x0F80, 0x0A00, 0x1B80, 0x1E00, 0x1400, 0x1180,
                ...
                0x0001};
        av_crc_table[3] = new int[]{0x0000, 0x2110, 0x4220, 0x6330, 0x8440, 0xA550, 0xC660, 0xE770,
        ...
        av_crc_table[7] = new int[]{0x000000, 0xFB4C86, 0x0DD58A, 0xF6990C, 0xE1E693, 0x1AAA15, 0xEC3319,
        ...
        av_crc_table[4] = new int[]{0x00000000, 0xB71DC104, 0x6E3B8209, 0xD926430D, 0xDC760413, 0x6B6BC517,
    ...
    av_crc_table[5] = new int[]{0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F,
    ...
    av_crc_table[6] = new int[]{0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241,
    ...
    public  static int[] av_crc_get_table(AVCRCId crc_id){
        if (!CONFIG_HARDCODED_TABLES) {
            switch (crc_id) {
                case AV_CRC_8_ATM:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_8_ATM.getValue());
                    break;
                case AV_CRC_8_EBU:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_8_EBU.getValue());
                    break;
                case AV_CRC_16_ANSI:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_ANSI.getValue());
                    break;
                case AV_CRC_16_CCITT:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_CCITT.getValue());
                    break;
                case AV_CRC_24_IEEE:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_24_IEEE.getValue());
                    break;
                case AV_CRC_32_IEEE:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_32_IEEE.getValue());
                    break;
                case AV_CRC_32_IEEE_LE:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_32_IEEE_LE.getValue());
                    break;
                case AV_CRC_16_ANSI_LE:
                    CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_ANSI_LE.getValue());
                    break;
                default:
                    //av_assert0(0);
            }
        }
        return av_crc_table[crc_id.getValue()];
    }

    public static void CRC_INIT_TABLE_ONCE(int id){
         //根据id保证多线程方法只执行一次
        //ff_thread_once(&id ## _once_control, id ## _init_table_once)
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_8_ATM.getValue(),      false,  8,       0x07);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_8_EBU.getValue(),      false,  8,       0x1D);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_ANSI.getValue(),    false, 16,     0x8005);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_CCITT.getValue(),   false, 16,     0x1021);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_24_IEEE.getValue(),    false, 24,   0x864CFB);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_32_IEEE.getValue(),    false, 32, 0x04C11DB7);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_32_IEEE_LE.getValue(), true, 32, 0xEDB88320);
        DECLARE_CRC_INIT_TABLE_ONCE(AVCRCId.AV_CRC_16_ANSI_LE.getValue(), true, 16,     0xA001);
    }
    public static void DECLARE_CRC_INIT_TABLE_ONCE(int id,boolean le,int bits,int poly) {
        _init_table_once(id,le,bits,poly);
    }

    static void _init_table_once(int id,boolean le,int bits,int poly){
        av_assert0(av_crc_init(av_crc_table[id], le, bits, poly, av_crc_table[id].length) >= 0);
    }
    public static void av_assert0(boolean s){
        System.out.println(s);
    }

    static int EINVAL = 22;
    public static int av_crc_init(int[] ctx, boolean le, int bits, int poly, int ctx_size){
        int i, j;
        int c;
        if (bits < 8 || bits > 32 || poly >= (1L << bits))
        return EINVAL;
        if (ctx_size != 32 * 257 && ctx_size != 32 * 1024)
            return EINVAL;
    ...
    public static int av_crc(int[] ctx, int crc,
                byte[] buffer, int length) {
        int end = buffer.length + length;
        int bufferIndex = 0;
        if(!CONFIG_SMALL) {
            if (!(ctx[256] == 0)) {
                while (((((long)buffer[bufferIndex] & 3) > 0) && buffer[bufferIndex] < buffer[end]))
                    crc = ctx[((byte) crc) ^ bufferIndex++] ^(crc >> 8);

                while (buffer[bufferIndex] <  buffer[end] - 3) {
                    crc ^= buffer[bufferIndex];buffer[bufferIndex] += 4;
        ...
        while (buffer[bufferIndex] < buffer[end])
            crc = ctx[((byte) crc) ^ buffer[bufferIndex++]] ^ (crc >> 8);
